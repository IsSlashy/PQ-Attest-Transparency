...
**Follow-ups documentés restants** (THREAT-MODEL §6) : timestamps/freshness sur STH (attaque par omission), log Merkle incrémental O(log n), persistance/rotation de clés, fédération de témoins réelle (réseau/gossip) ; **optionnel** `ChainAnchor` on-chain.
