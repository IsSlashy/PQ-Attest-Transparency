...
- **M7 — `ChainAnchor` on-chain** : *✅ FAIT.* Abstraction `Ledger`/`MockLedger`/`ChainAnchor` (rng-free, testée, démo scénario 6) + **vrai programme Solana** `onchain/programs/sth-anchor` (Anchor 0.32, immuabilité via `init`-once par epoch) **compilé en BPF et déployé sur un validateur localnet** (preuve dans `docs/CHAIN-ANCHOR.md`). Build/deploy faits en WSL (build-sbf Windows bloqué par un bug d'install des platform-tools).

**Follow-ups documentés restants** (THREAT-MODEL §6) : timestamps/freshness sur STH (attaque par omission), log Merkle incrémental O(log n), persistance/rotation de clés, fédération de témoins réelle (réseau/gossip), exercice client on-chain complet (anchor_root + read + rejet de conflit).
