...
→ **M0–M5 verts** (12 tests + smoke WASM ; audit adversarial passé et corrigé). MVP Web2 de référence : quote (racine-hw mockée + `QuoteVerifier`) → SLH-DSA STH → RFC6962 inclusion/consistance → X-Wing (HNDL) → co-signature de témoins avec détection d'équivocation client → vérifieur WASM ; `THREAT-MODEL.md` + `BENCHMARKS.md`. **Follow-ups documentés** (THREAT-MODEL §6) : câbler le quorum de témoins dans le WASM (top), timestamps/freshness sur STH, log Merkle incrémental, persistance/rotation de clés ; **optionnel** `ChainAnchor` on-chain.
