...
- **M5 — bench + honnêteté + durcissement** : *✅ FAIT.* `BENCHMARKS.md` (tailles vs classique sourcées + latences réelles ; reçu ~10 KB, sig ~123× Ed25519, vérif client ~0,14 ms) ; `THREAT-MODEL.md` (prouvé/supposé/mocké, exhaustif). **Audit adversarial multi-agent** (74 findings) → corrections : `QuoteVerifier` + racine-hw mockée + test négatif prouvant la tautologie du binding ; checks de longueur KEM ; consistance m=0 ; **détection d'équivocation côté client** dans `WitnessAnchor` (anti-split-view défense-en-profondeur) ; ancrage sur `(taille, racine)` ; scénario 5 « vrai gain ». 12 tests.
...
→ **M0–M6 verts** (12 tests + smoke WASM quorum ; audit adversarial passé et corrigé). MVP Web2 de référence : quote (racine-hw mockée + `QuoteVerifier`) → SLH-DSA STH → RFC6962 inclusion/consistance → X-Wing (HNDL) → co-signature de témoins avec détection d'équivocation client → **vérifieur WASM qui vérifie le quorum lui-même** (`verify_receipt_full`) ; `THREAT-MODEL.md` + `BENCHMARKS.md`.
- **M6 — quorum de témoins dans le WASM** : *✅ FAIT.* `verify_receipt_full` ingère un `CosignedSth` contre les clés témoins pinées + seuil (smoke : honnête=accept, falsifié/sous-quorum/sans-témoin=reject). Le navigateur ne fait plus confiance à une racine fournie.

- **M6 — quorum de témoins dans le WASM** : ✅ FAIT (voir plus haut).
- **M7 — `ChainAnchor` on-chain** : *✅ FAIT.* Abstraction `Ledger`/`MockLedger`/`ChainAnchor` (rng-free, testée, démo scénario 6) + **vrai programme Solana** `onchain/programs/sth-anchor` (Anchor 0.32, immuabilité via `init`-once par epoch) **compilé en BPF, déployé ET exercé de bout en bout** sur un validateur localnet (`anchor test` : publish → read → **rejet d'une 2e racine au même epoch** → nouvel epoch, `1 passing` ; preuve dans `docs/CHAIN-ANCHOR.md`). Build/deploy/test faits en WSL (build-sbf Windows bloqué par un bug d'install des platform-tools).
- **M8 — pinning des témoins on-chain** : *✅ FAIT.* Le client n'a plus à faire confiance au set de témoins fourni par le provider : `witness::witness_set_commitment` + `WitnessAnchor::new_checked` vérifient le set contre un **commitment immuable ancré on-chain** (instruction `anchor_witness_set`, PDA par log, `init`-once). Substitution de clés témoins → commitment ≠ chaîne → rejet. Réponse directe au point « critique » de la distribution de clés. 15 tests (dont 2 M8) + scénario 7 + `anchor test` **2 passing** (racine + witness-set). *Résiduel honnête : le check n'est pas encore câblé dans le vérifieur WASM ; la collusion du quorum genuine reste hors-scope.*

**Follow-ups documentés restants** (THREAT-MODEL §6) : timestamps/freshness sur STH (attaque par omission), log Merkle incrémental O(log n), persistance/rotation de clés, fédération de témoins réelle (réseau/gossip).
